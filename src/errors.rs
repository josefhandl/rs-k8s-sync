...
impl std::fmt::Display for KubernetesError {
  fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
    match self {
      ...
      KubernetesError::IoError { source } => write!(f, "Couldn't read file (io::Error). Source: {}", source),
      KubernetesError::ClientBuildError => write!(f, "Couldn't build client (isahc)"),
      KubernetesError::HttpClientBuildError { message} => write!(f, "Couldn't build http client (isahc): {}", message),
      KubernetesError::HttpClientRequestError => write!(f, "Couldn't build http client (isahc)"),
      KubernetesError::HttpClientParseResponseError { message } => write!(f, "Couldn't parse response from HTTP server: {}", message),
      KubernetesError::Base64DecodeError { source } => write!(f, "Couldn't decode base 64. Source: {}", source),
      ...
      KubernetesError::ApiRequestError { source } => write!(f, "API returned error: {}.", source),
    }
  }
}
