...
impl Config {
    pub fn load_config<P: AsRef<Path>>(path: P) -> Result<Config, std::io::Error> {
...
impl Cluster {
    pub fn load_certificate_authority(&self) -> Option<Result<Vec<u8>, std::io::Error>> {
...
impl AuthInfo {
    pub fn load_client_certificate(&self) -> Result<Vec<u8>, std::io::Error> {
        utils::data_or_file_with_base64(&self.client_certificate_data, &self.client_certificate)
    }

    pub fn load_client_key(&self) -> Result<Vec<u8>, std::io::Error> {
