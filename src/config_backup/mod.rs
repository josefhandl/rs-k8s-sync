...
impl Configuration {
    pub fn new(base_path: String, client: HttpClient) -> Self {
        Configuration {
...
/// ```
pub fn load_kube_config() -> Result<Configuration, std::io::Error> {
...
///     .expect("failed to load kubeconfig");
/// ```
pub fn load_kube_config_with(options: ConfigOptions) -> Result<Configuration, std::io::Error> {
    let kubeconfig = utils::kubeconfig_path()
        .or_else(utils::default_kube_path)
        .ok_or("Unable to load kubeconfig")?;
    ...
    let token = match &loader.user.token {
        ...
        //        let status = creds
        //            .status
        //            .ok_or("exec-plugin response did not contain a status")?;
        //        status.token
        //    } else {
        //        None
        //    }
...
/// ```
pub fn incluster_config() -> Result<Configuration, std::io::Error> {
    let server = incluster_config::kube_server().ok_or(format!(
