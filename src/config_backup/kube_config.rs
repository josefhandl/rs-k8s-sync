...
use crate::config_backup::apis::{AuthInfo, Cluster, Config, Context};
...
impl KubeConfigLoader {
    pub fn load<P: AsRef<Path>>(
        ...
        user: Option<String>,
    ) -> Result<KubeConfigLoader, std::io::Error> {
        ...
        let current_context = config
            ...
            .ok_or("Unable to load current context").unwrap();
        ...
        let cluster = config
            ...
            .ok_or("Unable to load cluster of current context");
        ...
        let user = config
            ...
            .ok_or("Unable to load user of current context");
        Ok(KubeConfigLoader {
            current_context: current_context.to_owned(),
            cluster: cluster.unwrap().to_owned(),
            user: user.unwrap().to_owned(),
        })
    }

    pub fn p12(&self, password: &str) -> Result<Pkcs12, std::io::Error> {
    ...
    pub fn ca(&self) -> Option<Result<X509, std::io::Error>> {
        let ca = self.cluster.load_certificate_authority()?;
        Some(ca.and_then(|ca| X509::from_pem(&ca).map_err(std::io::Error::from)))
    }
}